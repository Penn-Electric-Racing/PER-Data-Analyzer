"""Integration handler for MCP server."""
...
from perda.utils import integrate_over_time_range
...
TOOL_DEFINITIONS = {
    "integrate_variable_over_time": {
        "description": "Calculate the integral (area under the curve) of a variable over a time range. "
        "Useful for energy calculations (power×time), distance (speed×time), and total charge (current×time).",
        "inputSchema": {
            "type": "object",
            "properties": {
                "variable": {
                    "type": "string",
                    "description": "Full variable path (e.g., 'ams.pack.power')"
                },
                "start_time": {
                    "type": "integer",
                ...
                "time_unit": {
                    "type": "string",
                    "description": "Time unit for integration: 'ms' or 's' (optional, default: 's')",
                    "enum": ["ms", "s"]
                }
            },
            "required": ["variable"]
...
def handle_integrate_variable_over_time(arguments: dict) -> list[Content]:
    """Calculate integral of a variable over time."""
    ...
    try:
        ...
        # Format output
        output = f"Integral of '{data_instance.label}' (CAN ID: {data_instance.canid})\n"
        output += f"=" * 50 + "\n\n"
        output += f"Time Range:\n"
        ...
        output += f"\nInterpretation: This is the area under the curve,\n"
        output += f"representing the accumulated quantity over the time period."
